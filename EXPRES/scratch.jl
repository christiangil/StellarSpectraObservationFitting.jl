...
flat_initial_params, unflatten = flatten(raw_initial_params)
...
only(Zygote.gradient(objective ∘ unflatten, flat_initial_params))

l, back = Zygote.pullback(objective ∘ unflatten, flat_initial_params)
only(back(1))

using StellarSpectraObservationFitting; SSOF = StellarSpectraObservationFitting
...
d = SSOF.TestData(ones(4, 5), ones(4, 5), ones(4, 5), ones(4, 5))
