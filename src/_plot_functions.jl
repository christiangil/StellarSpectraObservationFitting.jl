...
plot_model_rvs!(plt, times, rvs, rvs_σ; label="", xlabel="", markerstrokewidth=1, kwargs...) = my_scatter!(plt, times, rvs; yerror=rvs_σ, label=label*" RVs, std: $(round(std(rvs), digits=3)), intra night std: $(round(SSOF.intra_night_std(rvs, times), digits=3))", xlabel=xlabel, markerstrokewidth=markerstrokewidth, kwargs...)
...
function plot_model_rvs(times_nu::AbstractVector{T}, model_rvs::AbstractVecOrMat{T}, model_rvs_σ::AbstractVecOrMat{T}, inst_times::AbstractVector{T}, inst_rvs::AbstractVector{T}, inst_rvs_σ::AbstractVector{T}, ccf_rvs::AbstractVector{T}; display_plt::Bool=true, kwargs...) where {T<:Real}
    ...
    my_scatter!(plt[1], inst_times, ccf_rvs .- median(ccf_rvs); label="CCF RVs,      std: $(round(std(ccf_rvs), digits=3)), intra night std: $(round(SSOF.intra_night_std(ccf_rvs, inst_times), digits=3))", alpha = 0.7, kwargs...)
    if display_plt; display(plt) end
    return plt
end
